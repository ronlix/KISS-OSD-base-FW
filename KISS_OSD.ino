...
#endif
#if defined(KISS_MIO) || defined(KISS_VTX)
...
void loop(){
  ...
  if(micros()-LastLoopTime > 10000){
    ...
    while(recBytes < minBytes && micros()-LastLoopTime < 50000){
      #define STARTCOUNT 2
      if(Serial.available()) serialBuf[recBytes++] = Serial.read();
      ...
      if(recBytes == minBytes){
         ...
         if(checksum == serialBuf[recBytes-1]){
           ...
           yaw = ((serialBuf[6+STARTCOUNT]<<8) | serialBuf[7+STARTCOUNT])/10;
           
           if(serialBuf[16+STARTCOUNT] != 0) wasArmed = 1;
